function getTopDangerous(){
  ...
  reverseGeocoding(result, 0, function(result){
    ...
    $("#resultTopDangerous").val(html);

    // update map
    filterPoints(acceptedTypes);

    window.layers.indicators.clearLayers();
    window.layers.indicators.addData(result);

    $("#downloadTopDangerous").prop("hidden", false);
...
function downloadTopDangerousResults(){
  downloadCsv("cruces_top_incidentes_repubikla", $("#resultTopDangerous").val());
}
...
function clearIndicators(){
  ...
  $("#resultTopDangerous").val("");

  $("#downloadTopDangerous").prop("hidden", true);
}
